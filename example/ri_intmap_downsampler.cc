...
int main(int argc, char **argv) {
  ...
  if(norm)
  {
    ...
    cv::imwrite(output, outRi);
    cv::imwrite(outputIntMap, outIntMap);
  }
  else
  {
    outRi = ri;
    ...
    cv::FileStorage riFile(output, cv::FileStorage::WRITE);
    riFile << "ri" << outRi;
    riFile.release();


    cv::FileStorage intMapFile(outputIntMap, cv::FileStorage::WRITE);
    intMapFile << "intmap" << outIntMap;
    intMapFile.release();
  }
