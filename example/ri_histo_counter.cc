...
int main(int argc, char **argv) {
  ...
  options.add_options()
    ...
    ("l, level", "total bins", cxxopts::value<float>())
  ...
  int level = parsedArgs["level"].as<float>();
  ...
  for(int i = 0; i < ri.rows; i++)
  {
    for(int j = 0; j < ri.cols; j++)
    {
      ...
      int idx = (int)((val - min) / (max - min) * level);
      histo[idx]++;
    }
  }
