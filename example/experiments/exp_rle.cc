...
int main(int argc, char **argv) {
  ...
  for(int prec = 0; prec < 6; prec++)
  {
    ...
    for(int idx = 0; idx < numScans; idx++)
    {
      ...
      st = getTsNow();
      riConverter.denormalizeRi(recNri, riMin, riMax, recRiFromNri);
      et = getTsNow();
      ...
      float PSNR = Metrics::calcPsnrBtwPcs(pcXyz, recPc1, 80);
      float CD   = Metrics::calcCdBtwPcs(pcXyz, recPc1);
      float PSNR2 = Metrics::calcPsnrBtwPcs(pcXyz, recPc2, 80);
