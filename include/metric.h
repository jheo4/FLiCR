...
float getDistance(const pcl::search::KdTree<pcl::PointXYZ> &tree, const pcl::PointXYZ &pt)
{
  const int k = 1;
...
float getMSE(PclPcXYZ pc1, PclPcXYZ pc2, float distThresh = std::numeric_limits<float>::max())
{
  int outliers = 0;
...
float calcPSNR(PclPcXYZ pc1, PclPcXYZ pc2, float pcMax)
{
...
float getImgMSSIM( const cv::Mat& ri1, const cv::Mat& ri2)
{
